...
**Not in this archive (still only on the author's side):** the original 1/12 "adversarial patch" PDF that seeded the r2 session, and the intermediate r2 outputs (`babai_motion_d3_revised.*`, the non-strengthened `source_annotated.*`, `REVISION_NOTES.md`, `PROVENANCE_INDEX.md`). None are needed for the verdict — the strengthened manuscript supersedes them all — but the author can add them here any time for lineage completeness.
